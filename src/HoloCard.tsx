...
const HoloCard = ({ isMobile = false }: HoloCardProps) => {
	...
	// )
	const borderTexture = useLoader(
		THREE.TextureLoader,
		'/cherubimon_border.png'
	);
	...
	useEffect(() => {
		...
		const handleMouseMove = (e: MouseEvent) => {
			mouseRef.current.x = ((e.clientX / window.innerWidth) * 2 - 1) * 3;
			mouseRef.current.y = (-(e.clientY / window.innerHeight) * 2 + 1) * 2;
		};
	...
	return (
      ...
      >
        ...
        />
			...
			{/* Card mesh */}
			<group ref={cardRef} scale={[0.5, 0.5, 0.5]}>
				{/* Background mesh */}
				<mesh
					ref={bgRef}
					position={[0, 0, -2.3]}
					...
					<holofoilNoiseMaterial
						...
						uBrightness={0.7}
						uDiagonalStrength={3.0}
						uFalloffDistance={1.5}
					/>
				</mesh>
				<mesh
					ref={cherubimonRef}
